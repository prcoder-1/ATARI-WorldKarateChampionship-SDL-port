...
and `$EE,y` the animation frame index; `$F0,y` is the next index. Each video frame the
index advances by one, and when it reaches `MOVE_FRAME_START[move+1]` the move is over
and the queued move `$EC,y` is taken. Per frame:
...
animation **backwards** (`$54E5`), `$08` the strike is live, `$10` input may be taken,
...
**Movement and the arena.** `$5509` adds `FRAME_VELX[frame]` to `x`, signed by facing,
then clamps: for facing 0 the box is `[x, x + SHAPE_WIDTH]`, for facing 1 it is
`[x + $24 - SHAPE_WIDTH, x + $24]`, and it must stay inside `$10..$AE`.
...
Runs once per CPU fighter per frame, immediately before that fighter's state machine.
...
follows sees an empty queue. Ported to `ai.h`; the tables are generated into
`generated/frames.h`.
